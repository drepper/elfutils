...
main (int argc, char *argv[])
{
  ...
  if (strcmp (argv[1], "gnu") == 0)
    compress = ELF_ZSCN_T_GNU;
  else
    compress = ELF_ZSCN_T_ELF;
      ...
      while ((scn = elf_nextscn (elf, scn)) != NULL)
	{
	  ...
	  if (idx == strndx)
	      ...
	      if (orig_buf == NULL)
		{
	      ...
	      if (gelf_zscn_compress (scn, &mem, compress,
				      ELFCOMPRESS_ZLIB) == NULL)
		{
		...
		{
		  printf ("gelf_zscn_[un]compress failed for section %zd: %s\n",
			  idx, elf_errmsg (-1));
		  return -1;
		}
	      d = elf_getdata (scn, NULL);
	      new_size = shdr->sh_size;
